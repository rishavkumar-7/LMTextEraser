...
def main(args) -> None:
    ...
    # 111111111111111111111111111111111111111111111111111111111111111111111111111 bleu_dataset["original"]=get_original_dataset()
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
# from createDataset import create_forget_dataloader , create_retain_dataloader ,get_truthfulQA_answers_plaintext # get_original_dataset
# from loss import get_answer_loss,compute_kl,get_answer_loss1,get_rand_ans_loss
...
#         for bad_batch, normal_batch in zip(train_bad_loader, train_normal_loader):
#             ############ GA on answer only. ############
#             # bad_loss = get_answer_loss("ga", bad_batch, 
#             #                            # model,
#             #                            pretrained_model,
#             #                            device=device)         #gets lossss
#             # # _bad_loss.append(bad_loss.item())
#             # # bad_loss_original=get_answer_loss("ga",bad_batch,pretrained_model,device=device)
#             retain_loss=get_answer_loss1("ga",
#                                          bad_batch,
#                                          # normal_batch,
#                                          # model,
#                                          pretrained_model, # pretrained model doesn't gives error but when passed  model (finetuned model) gives error
#                                          device=device)
...
#     model.save_pretrained(args.model_save_dir, from_pt=True,force_download=True)
#     logging.info("Unlearning finished")
#     plot_total_loss_graph(bad=_bad_loss,retain=_retain_loss,normal=_normal_loss,total=_total_loss,name="Total_")
...
# def plot_total_loss_graph(bad,retain,normal,total,name):
